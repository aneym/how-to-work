...
2. Run `npx how-to-work@latest check` (add `--online` to compare against the latest published engine). It validates engine-version drift and config-schema drift and prints the exact `init` / `init --migrate` fix command. If it exits non-zero, run the printed command before proceeding.
3. `answerGate.mode` (`none | local | hermes`) decides how the live gate behaves — read it before deciding whether to start a server-backed gate or fall back to copy-only.
...
```bash
npx how-to-work@latest grill ask --doc <slug> --base <answerGate.base>
```
...
- **`hermes`** — a host-private delivery overlay (an `onAnswer` callback the host wires up); the public engine ships no such delivery, so this mode only does something where the host repo provides it.
