...
**`/htw` is the canonical invocation** — a dispatcher that bootstraps repo context and routes intent ("write a doc on X", "scope Y", "grill me", "send it") to the right protocol; see the sibling `htw` skill. This file is the full workflow law that `/htw` routes into. `/how-to-work` and `/how-to` are long-form aliases; the old `how-we-work` name is a legacy compatibility alias only — never choose it for new installs, docs, or prompts.
...
2. Validate / sync: run `npx --yes github:aneym/how-to-work doctor` — it checks engine-version drift, config schema, interface shims, stale renders, catalog gaps, and stage divergence in one pass, and `doctor --fix` applies every mechanical repair. If there is no config yet, `npx --yes github:aneym/how-to-work init` stamps one. (Always invoke the GitHub form `npx --yes github:aneym/how-to-work …` — the npm registry copy is stale; `npx how-to-work@latest` would DOWNGRADE the engine.)
3. Ensure local agent interfaces exist: run `npx github:aneym/how-to-work interfaces` after setup or migration. This installs project-local `how-to-work` skills plus `/how-to-work` and `/how-to` command shims for Codex, Claude, and generic agent runtimes.
...
Almost all `/how-to-work` specing work should create or update an HTML surface the author can follow along in. Do not keep substantial specing only in chat, markdown, or internal notes unless the author explicitly asks for a text-only pass.
...
- **Main workflow surface:** route bot-level / cross-repo / personal How To Work docs and PRDs to the repo your config designates as the workflow home; put product/repo-specific PRDs in the owning repo. Do not pick a destination solely because it is the current working directory.

Progress is measured by lifecycle **stage** — the canonical sequence Working doc → Draft PRD → Ready for approval → Approved → In execution → Done (sourced from config `doc.stagesPath`, or the engine's built-in 6-stage lifecycle when null) — rendered as a monochrome segmented stage bar, or a circular stage ring in tight spaces like a sidebar; a percent-complete is deprecated and at most secondary. **`state.json` is the stage authority; frontmatter is a derived mirror.** Move stages ONLY with `npx --yes github:aneym/how-to-work stage set <slug> "<stage>"` — it updates state.json, frontmatter, ledger, the rendered HTML, and the catalog in one transaction, and `htw verify` fails any doc whose surfaces diverge.

**Attention-volume law (companion to attention ordering):** what renders first matters, and so does *how much* renders. The engine enforces the floor — agent-call decisions collapse behind a count chip (max 5 author decisions visible), ledgers roll up past the newest 3 days, and long tabs grow an in-tab section nav automatically. Your job as author is the ceiling: a tab is a readable brief, not a dump — split sections into tabs when one outgrows a few screens, summarize rather than accrete, and never present a several-thousand-word single scroll as finished work. Omit empty sections entirely (no "Blockers: none" filler).
...
- **After posting questions, START THE ANSWER GATE — never make the author copy-paste.** Run `npx --yes github:aneym/how-to-work grill ask --doc <slug> --apply` (foreground when you can wait, else background). It opens the ask so the doc's question section lights up ("the agent is waiting for your answers"), polls the gate, and — with `--apply` — writes the answers into every surface the moment the author clicks "Submit to agent". Gate behavior follows `answerGate.mode`: in `local`/`custom` mode, posting questions without starting the gate strands the submitted answers in the store (recover via `GET <answerGate.base>/result?key=<docKey>`); in `none` mode there is no server, so the Copy-answers button is the equivalent and a missing live gate is expected, not a bug.
- **Answers that arrive in chat go through `htw grill resolve <slug>`** (pipe the pasted packet / shorthand / gate JSON to stdin). It writes the `answer:` fields, appends `[Decided]` rows to Decisions, appends ledger events, bumps state, and re-renders — one command, every surface. Never apply answers by hand-editing.
- **Every surface updates in the same pass — via the atomic commands, not memory.** `grill resolve`, `stage set`, and `ledger add` each mutate source + state + ledger + rendered HTML + catalog together, and `htw verify` fails stale or divergent surfaces. If you touched `.doc.md` directly (content edits), re-render immediately; `htw render` auto-registers.
...
- If asked to scope `/doc`, `/how-to-work`, `/how-to`, agent-workflow docs, or workflow infrastructure while the shell is sitting in another checkout, route to the configured workflow home by default — not to the current working directory solely because that is where you are.
